  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#0f172a] flex flex-col items-center justify-center p-6 text-safety-orange overflow-hidden">
      ...
      <div className="text-center z-10">
        ...
        <div className="font-display text-xl font-bold tracking-widest min-h-[32px] flex items-center justify-center gap-1">
          <span>{displayText}</span>
          <span className="w-2 h-5 bg-safety-orange animate-pulse"></span>
        </div>
        
        {/* Progress Bar */}
        <div className="mt-8 w-64 h-1 bg-safety-orange/10 relative overflow-hidden mx-auto">
          <div 
            className="absolute top-0 left-0 h-full bg-safety-orange transition-all duration-500 ease-out"
            style={{ width: `${((currentStep + (isTypingComplete ? 1 : 0)) / BOOT_MESSAGES.length) * 100}%` }}
          ></div>
        </div>
      </div>

      {isTypingComplete && isReady && (
        <div className="mt-16 animate-fade-in flex flex-col items-center gap-4">
          <button 
            onClick={onEnter}
          ...
          <span className="text-[9px] font-display font-bold text-safety-orange/40 tracking-[0.2em] animate-pulse">AUTHORIZATION_GRNTD</span>
        </div>
      )}
