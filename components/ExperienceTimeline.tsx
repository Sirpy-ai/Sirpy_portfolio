...
const ExperienceTimeline: React.FC = () => {
  ...
  const displayExperiences = expanded ? EXPERIENCES : EXPERIENCES.slice(0, 3);

  return (
    <section id="timeline" className="max-w-4xl mx-auto font-display">
      <div className="flex items-center gap-6 mb-16">
        <div className="flex flex-col">
          <span className="text-safety-orange text-[10px] font-bold tracking-[0.4em] mb-1">SECTION 03</span>
          <h2 className="text-3xl font-black text-white tracking-tighter uppercase m-0">MISSION LOGS</h2>
        </div>
        ...
        >
          {expanded ? 'ARCHIVE DATA' : 'RETRIEVE ALL LOGS'}
        </button>
      </div>
          ...
          <div key={idx} className="relative pl-16 group">
            ...
            <div className="flex flex-col gap-4 p-6 bg-cad-gray/5 border border-cad-gray/20 group-hover:border-safety-orange/30 transition-all duration-500">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <span className="text-[10px] font-bold text-safety-orange tracking-[0.2em]">{exp.range.toUpperCase()}</span>
                <span className="text-[8px] px-2 py-0.5 bg-cad-gray/20 text-cad-gray border border-cad-gray/30 font-bold tracking-widest uppercase">MISSION BRANCH: {exp.branch}</span>
              </div>

              <div>
                <h3 className="text-xl font-black text-white group-hover:text-safety-orange transition-colors m-0 uppercase">{exp.role}</h3>
              ...
              <p className="text-sm text-slate-400 font-sans leading-relaxed m-0">{exp.action}</p>
